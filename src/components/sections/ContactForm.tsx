...
const inputClass =
  'w-full bg-navy-950 border border-navy-700 rounded-lg px-4 py-3 text-white text-sm placeholder-slate-500 focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/30 transition-colors'
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label="Email Address" error={errors.email?.message}>
          <input {...register('email')} type="email" placeholder="john@smithhvac.com" className={inputClass} />
        </Field>
      ...
      <Field label="Company Name" error={errors.company?.message}>
        <input {...register('company')} placeholder="Smith Heating & Cooling" className={inputClass} />
      </Field>
      ...
      <Button type="submit" size="lg" className="w-full" disabled={submitting}>
        {submitting ? (
          ...
          <>Book My Free Revenue Recovery Audit<Send className="w-4 h-4" /></>
        )}
      </Button>
